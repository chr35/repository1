...
<html>
  <head>
	...
	<%	
		...
		for(int i=0;i<list.size();i++){
			...
			tlist+="<tr>";
			tlist+="<td>"+m.get("classid")+"</td>";
			tlist+="<td>"+m.get("classname")+"</td>";
			tlist+="<td><a href='jsp/background/xxwh/classinfoEdit.jsp?tid="+m.get("id")+"'>编辑</a> "+
				"<a href='javascript:delclassinfo("+m.get("id")+")'>删除</a></td>";
			tlist+="</tr>";
		}
  ...
  <body>
    ...
    <div class="condition">
    	<form action="Xxwh.do?method=classinfosel" method="post">
    		<table>
    			<tr>
    				<td>班级编号:</td><td><input type="text" name="cclassid"/></td><td>班级名称:</td><td><input type="text" name="cclassname"/></td>
    			</tr>
    		</table>
    		<input class="btn" type="submit" value="查询"/> <input class="btn" type="button" onclick="location.href='jsp/background/xxwh/classinfoAdd.jsp'" value="添加"/>
    	</form>
    </div>
    <div class="tlist">
    	<table class="tlisttable">
    		<tr>
    			<th>班级编号</th><th>班级名称</th><th>操作</th>
    		</tr>
    		<%=tlist %>
    	</table>
    	<div class="divPage">
    ...
    function delclassinfo(id){
    	if(confirm("你确定要删除这条记录吗？")){
    		location.href="Xxwh.do?method=classinfodel&id="+id;
    	}
    }
    </script>
  </body>
