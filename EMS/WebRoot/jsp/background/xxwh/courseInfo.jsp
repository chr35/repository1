...
<html>
  <head>
	...
	<%	
		...
		for(int i=0;i<list.size();i++){
			...
			tlist+="<tr>";
			tlist+="<td>"+m.get("courseid")+"</td>";
			tlist+="<td>"+m.get("coursename")+"</td>";
			tlist+="<td>"+m.get("courseType")+"</td>";
			tlist+="<td>"+m.get("credit")+"</td>";
			tlist+="<td><a href='jsp/background/xxwh/courseEdit.jsp?tid="+m.get("id")+"'>编辑</a> "+
				"<a href='javascript:delcourse("+m.get("id")+")'>删除</a></td>";
			tlist+="</tr>";
		}
  ...
  <body>
    ...
    <div class="condition">
    	<form action="Xxwh.do?method=coursesel" method="post">
    		<table>
    			<tr>
    				<td>课程编号:</td><td><input type="text" name="cuserid"/></td><td>课程名称:</td><td><input type="text" name="cname"/></td>
    			</tr>
    		</table>
    		<input class="btn" type="submit" value="查询"/> <input class="btn" type="button" onclick="location.href='jsp/background/xxwh/courseAdd.jsp'" value="添加"/>
    	</form>
    </div>
    <div class="tlist">
    	<table class="tlisttable">
    		<tr>
    			<th>课程编号</th><th>课程名称</th><th>课程类型</th><th>学分</th><th>操作</th>
    		</tr>
    		<%=tlist %>
    	</table>
    	<div class="divPage">
    ...
    function delcourse(id){
    	if(confirm("你确定要删除这条记录吗？")){
    		location.href="Xxwh.do?method=coursedel&id="+id;
    	}
    }
    </script>
  </body>
